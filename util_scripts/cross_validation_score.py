...
def cv_score(base_path, partial_sub=True):
    submissoin = pd.read_csv(f'{base_path}/submission.csv')
    solution = pd.read_csv(f'{base_path}/solution.csv')
    if partial_sub:
        sub_rows = submissoin['row_id'].values
        solution = solution[solution['row_id'].isin(sub_rows)]
        solution = solution.reset_index(drop=True)
    cv_score = score(solution, submissoin, 'row_id', 1)
    return cv_score
