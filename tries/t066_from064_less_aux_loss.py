...
class CustomLoss(nn.Module):
    ...
    def forward(self, pred, label):
        ...
        return (class_loss + seg_loss * 0.4), {'class_loss': class_loss, 'seg_loss': seg_loss, **class_loss_dict, **seg_loss_dict}
...
class RSNAModel(nn.Module):
    def __init__(self, model_name):
        ...
        self.head = nn.Linear(encoder_channels[-1], 6)
...
class LocDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        for level in range(5):
            one_locs_label = []
            select_instance_numbers = []
            for condition in conditions:
                instance_number = locs_info[condition][level][0]
                select_instance_numbers.append(instance_number)
                one_locs_label.append(locs_info[condition][level][1:])
            select_image.append(image[:, select_instance_numbers[np.random.randint(2)], ...])
        ...
        locs_label_2d = torch.stack(locs_label_2d, dim=0)
    ...
    def _query_label(self, study_id):
        ...
        for level in levels: # !!! ATTENTION: level and condition is swapped here.
            for condition in conditions:
                tmp = [0.] * 3
