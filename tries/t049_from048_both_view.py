...
class CustomLoss(nn.Module):
    ...
    def forward(self, pred, label):
        ...
        cls_loss = self.cls_loss(pred[0], cls_label)
        importance_loss = self.importance_loss(pred[1], importance_label)
        return cls_loss + importance_loss, {'cls_loss': cls_loss, 'importance_loss': importance_loss}
...
def get_dicom_volume(subfolder_path):
    ...
    slice_position = []
    if os.path.isfile(os.path.join(subfolder_path, 'data.pt')) and False:
        ...
        for i, file in enumerate(dicom_files):
            ...
            if x.shape == volume.shape[1:]:
                ...
                slice_position.append(metadata['Image Position (Patient)'][0])
            else:
                volume = volume[:i]
                break
...
def main(): 
    ...
    df_co = get_df_co()
    data = get_data_w_series(df_series, drop_rate=0.0)
