...
class CustomLoss(nn.Module):
    ...
    def forward(self, pred, label):
        ...
        return class_loss + locs_loss * 0.05 + js_div, result_dict
...
class RSNADataset(Dataset):
    def __init__(self, df, df_for_label):
        ...
        self.data = {}
        self.conditions = ['spinal_canal_stenosis_l1_l2', 'spinal_canal_stenosis_l2_l3', 'spinal_canal_stenosis_l3_l4', 'spinal_canal_stenosis_l4_l5', 'spinal_canal_stenosis_l5_s1']
    ...
    def _apply_augment(self, image, locs):
        ...
        for idx, condition in enumerate(self.conditions):
            if condition not in locs:
                continue
            locs[condition] = [locs[condition][0], transformed['keypoints'][idx][0] / 384, transformed['keypoints'][idx][1] / 384]
        
        return transformed_volume, locs
...
def get_coordinates_from_series_id(series_id, filepath):
...
def get_df_series(filter='Sagittal T2'):
    ...
    df_series['locs'] = df_series.apply(lambda x : get_coordinates_from_series_id(x['series_id'], x['filepath']), axis=1)
    return df_series
