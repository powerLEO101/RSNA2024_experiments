...
class RSNADataset(Dataset):
    ...
    def _make_seg_label(self, volume, locs):
            ...
            for i in range(-length + 1, length):
                for j in range(-length + 1, length):
                    if in_bound(x + i) and in_bound(y + j):
                        seg_label[condition_id, z, x + i, y + j] = 1
    ...
    def _make_coordinates(self, locs):
        coordinates = []
        conditions = self.conditions
        for condition in conditions:
            if condition not in locs:
    ...
    def _apply_augment(self, image, locs):
        ...
        for one_image in image:
            transformed_volume.append(torch.tensor(A.ReplayCompose.replay(replay, image=one_image.float().numpy())['image']))
        ...
        for idx, condition in enumerate(self.conditions):
            if condition not in locs:
                continue
            new_locs[condition] = [locs[condition][0], transformed['keypoints'][idx][0] / 384, transformed['keypoints'][idx][1] / 384]
...
def get_df_series(filter='Axial'):
    bad_series_ids = [3401861580, 1364910156, 4140564625,  790180412, 3242313646,
                      4075111689,  519827232, 2594153114, 3101981332,  178314290,
                      396937199, 2603019387,  533252904, 3451074679, 1328374636,
                      2051789894, 1133656256, 2440822236, 4191635045, 1297079442,
                      3366910731, 2683794967, 3708423406, 4151611107, 3365980706,
                      304087230, 1935317419, 4195187002,  890756726,  458336097,
                      1415702790, 3238050001, 3911532570, 1519537115, 4045988081,
                      2216414062, 4190692765, 2479444365,  807064932, 4033318924,
                      549563660]
    df_series = pd.read_csv(f'{project_paths.base_path}/train_series_descriptions.csv')
    df_series = df_series[~df_series['series_id'].isin(bad_series_ids)].reset_index(drop=True)
...
def main(): 
    ...
    #df = get_df_series()
    df = pd.read_csv('/media/workspace/RSNA2024_input/rsna-2024-lumbar-spine-degenerative-classification/0915axial_df.csv')
