...
class Model(nn.Module):
    def __init__(self, backbone=None, segtype='unet', pretrained=True):
        ...
        g = self.encoder(torch.rand(1, 2, 64, 64))
        ...
        self.segmentation_head = nn.Conv2d(decoder_channels[n_blocks-1], 15, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1))
...
class LocDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        label = self._get_locs_label_3d(locs_info, len(image))
...
def main(): 
    ...
    save_weights = []
    for fold_n in range(2, config['folds']):
