...
class SegmentDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        result = self.resize(image=image.numpy().astype('f'), keypoints=meta['keypoints'])
        image = result['image']
        kps = result['keypoints']

        strong_labels = [] # TODO strong label could use full 5 labels for each slice, and only filter unneccesary ones when calculating loss
        images = []
        for i in range(0, image.shape[0], self.step_size):
            if i + self.window_len > image.shape[0]:
                break
            images.append(image[i : i + self.window_len, :])
            strong_label = [0] * 5
            for j in range(10):
                if meta['have_keypoints'][j] == False:
                    continue
        ...
        return {
            ...
            'level_label': torch.tensor(level_label, dtype=torch.float32),
            #'masks': masks,
