...
class SegmentDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        final_images = torch.zeros(10, 8, *self.image_size)
        for i in range(-5, 5):
            chosen_index = midpoint + i
            if chosen_index < 0 or chosen_index >= meta['total_instance_number']:
                continue

            image = self._get_slice(meta, chosen_index).float()
            masks = self._get_keypoints_pred(meta['series_id'], midpoint)
...
def train_one_fold(train_loader, valid_loader, fold_n):
    ...
    model_dict = '/media/workspace/RSNA2024_checkpoints/t035_from034_pretrain_coor.pt'
    ...
    criterion = PerLevelCrossEntropyLoss()
