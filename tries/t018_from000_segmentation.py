...
def train_one_fold(train_loader, valid_loader, fold_n):
    ...
    criterion = losses.PerLevelDiceLoss(device)
...
def get_loaders(df, df_co, data, fold_n):
    ...
    train_set = keypoints.SegmentDataset(df, train_df, data, augment_level=1)
    valid_set = keypoints.SegmentDataset(df, valid_df, data, augment_level=0)
...
def main(): 
    ...
    df_co.to_csv('./{file_name}_df_co.csv')
