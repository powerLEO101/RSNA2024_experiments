...
config = {
    ...
    'checkpoint_freq': 10 # no checkpoint in the middle
}
...
def valid_one_epoch(model, loader, criterion, optimizer, lr_scheduler, epoch, accelerator):
    ...
    for step, batch in bar:
        # B C X Y
        image, label = batch['image'], batch['loc']
        have_label = batch['have_keypoints']
        with torch.no_grad():
...
def train_one_fold(train_loader, valid_loader, fold_n):
    model = models.SegmentationModel(pool_kernel_size=4)
    accelerator.print(f'Training for {file_name} on FOLD #{fold_n}...')
    criterion = losses.PerLevelDiceLoss(device, image_size=[64, 64])
...
def get_loaders(df, df_co, data, fold_n):
    ...
    train_set = keypoints.SegmentDataset(df, train_df, data, augment_level=1, rough_pos_factor=4)
    valid_set = keypoints.SegmentDataset(df, valid_df, data, augment_level=0, rough_pos_factor=4)
...
def main(): 
    ...
    df_co = keypoints.get_df_co(df_co)
