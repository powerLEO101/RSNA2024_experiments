...
def train_one_fold(train_loader, valid_loader, fold_n):
    model = models.ThreeViewModel(model_name=config['model_name'], in_chans=3)
    accelerator.print(f'Training for {file_name} on FOLD #{fold_n}...')
    criterion = losses.TwoWayLoss(w2=0)
    ...
    for epoch in range(config['epoch']):
        training.train_one_epoch_w_pos(model=model, 
                                 loader=train_loader, 
                                 criterion=criterion, 
                                 optimizer=optimizer, 
        ...
        training.valid_one_epoch_w_pos(model=model, 
                                 loader=valid_loader, 
                                 criterion=criterion, 
                                 optimizer=optimizer, 
                                 lr_scheduler=lr_scheduler, 
...
def get_loaders(df, data, fold_n):
    ...
    valid_set = datasets.RSNADataset(valid_df, data, rough_pos_factor=32, severity_weights=[1, 1, 1])
    train_loader =  DataLoader(train_set, 
                                batch_size=config['batch_size'], 
                                shuffle=True, 
                                num_workers=15 if not 'LOCAL_TEST' in environ else 4, 
                                pin_memory=False)
